...
function App() {
  const initialMembers = [
    { isim: "Emre", email: "emre@workintech.com.tr", sifre: "pass123", kvkk: true },
    { isim: "Ömer", email: "omer@workintech.com.tr", sifre: "pass123", kvkk: true },
    { isim: "Erhan", email: "erhan@workintech.com.tr", sifre: "pass123", kvkk: true },
  ];
  ...
  return (
    ...
    <>
      <div className='block w-full'>
        <Form addMember={addMemberToTheTeam}
          editMember={updateMemberToTheTeam}
          duzenlenecekUye={editingMember} />
        <img className='h-10 relative -left-12 mb-6 dark:grayscale dark:invert' src="https://workintech.com.tr/assets/logo-light-c0bded94.svg" alt="Workintech Logo" />
        <h1 className='text-2xl font-extrabold tracking-tight text-slate-900 dark:text-slate-100'>Üye Kayıt Formu</h1>
        {members.length > 0 && (

          <div className='my-6 block'>
            {members.map((member, i) => (

              <div key={i} className="mt-2 flex items-center justify-between gap-x-3">
                ...
                <p className='text-base font-semibold leading-7 text-gray-900 dark:text-gray-100 dark: text-gray-50 text-left'>{member.isim}</p>{member.email}
                <button
                  type="button"
